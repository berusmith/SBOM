...
**8-per-iter signal commentary**: per §K closing meta-rule "the cumulative
count of K invocations per iter is itself a useful signal" — 8 invocations
across one PR remains in healthy band (not 0 = K is dead letter; not 10+
= plan too imprecise to follow).  All 8 correctly blocked an instruction
that, if executed naively, would have fixed an error in production-commit
form.  Notable: **4 of 8 (#3, #6, #7, #8)** fired across phase boundaries
(F-stage → sanity-sweep, Phase-10 ADR pre-write, Phase-10 merge prep,
post-merge maintenance), confirming K.6 ("§K applies to all observable
facts, not only code contents") covers cross-phase work.
