...
class IEC3_3Report(CjkPDF):
    ...
    def header(self):
        ...
        self.cell(0, 7, f"IEC 62443-3-3 System Security Report  |  {self.t(self.product)} {self.t(self.version)}", align="L")
        self.ln(1)
...
def _req_row(pdf, req_id, clause, title, status_label, color):
    ...
    pdf.sf("", 8)
    pdf.cell(96, 6, pdf.t(f" {title}"), fill=True)
