...
| dtrack_project_uuid | String nullable | 保留欄位（未來 D-Track 整合） |
...
| old_status | String nullable | 變更前狀態 |
| new_status | String | 變更後狀態 |
| changed_by | String nullable | 操作者（使用者名稱） |
...
| role | String | `admin` / `analyst`（預留多角色擴充） |
